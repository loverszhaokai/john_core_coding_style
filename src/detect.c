...
int main(int argc, char **argv)
{
	...
	size_log = 0;
	while (sizeof(long) * 8 != 1 << size_log)
		size_log++;
	...
	printf("#define ARCH_WORD\t\t\tlong\n"
	    ...
	    size_log,
	    (int)(sizeof(long) * 8),
	    (int)(*(char *)&value), (sizeof(int) > 4) ? 1 : 0);

	puts("#define CPU_DETECT\t\t\t0\n\n" "#define DES_ASM\t\t\t\t0");

	switch (argv[1][0]) {
	case '1':
		puts("#define DES_128K\t\t\t0\n"
		    "#define DES_X2\t\t\t\t0\n"
		    "#define DES_MASK\t\t\t0\n"
		    "#define DES_SCALE\t\t\t1\n" "#define DES_EXTB\t\t\t0");
		break;
	...
	printf("#define DES_COPY\t\t\t%c\n"
	    ...
	    "#endif\n",
	    argv[2][0],
	    argv[3][0], argv[4][0], argv[5][0], argv[6][0], argv[7][0]);
