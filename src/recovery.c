...
void rec_save(void)
{
	...
	if (fseek(rec_file, 0, SEEK_SET))
		pexit("fseek");
#ifdef __CYGWIN32__
	if (ftruncate(rec_fd, 0))
		pexit("ftruncate");
#endif
	...
	fprintf(rec_file, "%u\n%u\n%x\n%x\n%x\n%x\n%x\n%x\n%x\n"
	    ...
	    status.compat,
	    status.pass,
	    status_get_progress ? status_get_progress() : -1, rec_check);

	if (rec_save_mode)
		rec_save_mode(rec_file);

	if (ferror(rec_file))
		pexit("fprintf");

	if ((size = ftell(rec_file)) < 0)
		pexit("ftell");
	if (fflush(rec_file))
		pexit("fflush");
	if (ftruncate(rec_fd, size))
		pexit("ftruncate");
#ifndef __CYGWIN32__
...
void rec_restore_args(int lock)
{
	...
	rec_version = 0;
	if (!strcmp(line, RECOVERY_V4))
		rec_version = 4;
	else if (!strcmp(line, RECOVERY_V3))
		rec_version = 3;
	else if (!strcmp(line, RECOVERY_V2))
		rec_version = 2;
	else if (!strcmp(line, RECOVERY_V1))
		rec_version = 1;
	else if (strcmp(line, RECOVERY_V0))
	...
	if (fscanf(rec_file, "%u\n%u\n%x\n%x\n",
		...
		&status.guess_count, &status.combs.lo, &status.combs.hi) != 4)
		rec_format_error("fscanf");
	...
	if (rec_version >= 4) {
		if (fscanf(rec_file, "%x\n%x\n%x\n%x\n%x\n%d\n",
			...
			&status.crypts.hi,
			&status.cands.lo,
			&status.cands.hi, &status.compat) != 6)
			rec_format_error("fscanf");
	} else {
